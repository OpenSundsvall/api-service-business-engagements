package se.sundsvall.businessengagements.integration;
...
public abstract class BusinessEngagementsHealthIndicator implements HealthIndicator {
    ...
    public BusinessEngagementsHealthIndicator(String integrationName) {
    ...
    public void resetHealth() {
        LOG.info(LOG_INTEGRATION_RESET, integrationName);
        currentHealth = Health.unknown()
                .withDetail(STATUS_DESCRIPTION, NEVER_CALLED)
    ...
    public void setUpStatus() {
        if (!currentHealth.getStatus().equals(Status.UP)) {
            LOG.info(LOG_INTEGRATION_WORKING, integrationName);
    ...
    private void setCountersAndTimeStampAndLogForOutOfService() {
        if (!currentHealth.getStatus().equals(Status.OUT_OF_SERVICE)) {
            LOG.warn(LOG_INTEGRATION_OUT_OF_SERVICE, integrationName);
